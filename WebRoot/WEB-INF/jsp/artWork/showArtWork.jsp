			...
			<div align="center" class="main">
				<div id="func">
					<c:if test="${user.type == 1}">
						<form
							action="${pageContext.request.contextPath}/readExcelArtWork"
					...
					<form
						action="${pageContext.request.contextPath}/exportArtWork"
						method="post" class="function">
						<input type="submit" value="导出" id="lg-form">
					</form>
					<form
						action="${pageContext.request.contextPath}/findArtWork"
						method="post" class="function">
						<input type="submit" value="查询" id="lg-form">
					</form>
					<form
						action="${pageContext.request.contextPath}/addArtWork"
						method="post" class="function">
					...
					<table>
						...
						<tbody>
							<c:forEach items="${entryList}" var="entry">
								<tr align="center">
									<td><a href="showArtWorkDetail?id=${entry.id}">${entry.resultsName}</a></td>
									...
									<td><a href="editArtWork?id=${entry.id}">编辑</a> <a
										href="showDownloadArtWork?id=${entry.id}">下载</a> <c:if
											test="${user.type == 1}">
											<a href="dropArtWork?id=${entry.id}">删除</a>
										</c:if></td>
								</tr>
							</c:forEach>
						</tbody>
					...
					<tfoot>
						<tr>
							<td colspan="5"><form action="showArtWork" method="post"
									id="navigatorForm">
									<a href="showArtWork?pageNumber=${1}">首页</a>
									<c:if test="${pageNumber>1}">
										<a href="showArtWork?pageNumber=${pageNumber-1}">上一页</a>
									</c:if>
									...
									<c:if test="${pageNumber<totalPages}">
										<a href="showArtWork?pageNumber=${pageNumber+1}">下一页</a>
									</c:if>
									<a href="showArtWork?pageNumber=${totalPages}">末页</a>
								</form></td>
						</tr>
					</tfoot>
				</div>

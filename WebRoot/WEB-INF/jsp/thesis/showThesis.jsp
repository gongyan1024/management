			...
			<div align="center" class="main">
				<div id="func">
					<c:if test="${user.type == 1}">
						<form
							action="${pageContext.request.contextPath}/readExcelThesis"
					...
					<form
						action="${pageContext.request.contextPath}/exportThesis"
						method="post" class="function">
						<input type="submit" value="导出" id="lg-form">
					</form>
					<form
						action="${pageContext.request.contextPath}/findThesis"
						method="post" class="function">
						<input type="submit" value="查询" id="lg-form">
					</form>
					<form
						action="${pageContext.request.contextPath}/addThesis"
						method="post" class="function">
					...
					<table>
						...
						<tbody>
							<c:forEach items="${entryList}" var="entry">
								<tr align="center">
									<td><a href="showThesisDetail?id=${entry.id}">${entry.thesisName}</a></td>
									...
									<td><a href="editThesis?id=${entry.id}">编辑</a> <a
										href="showDownloadThesis?id=${entry.id}">下载</a> <c:if
											test="${user.type == 1}">
											<a href="dropThesis?id=${entry.id}">删除</a>
										</c:if></td>
								</tr>
							</c:forEach>
						</tbody>
					...
					<tfoot>
						<tr>
							<td colspan="5"><form action="showThesis" method="post"
									id="navigatorForm">
									<a href="showThesis?pageNumber=${1}">首页</a>
									<c:if test="${pageNumber>1}">
										<a href="showThesis?pageNumber=${pageNumber-1}">上一页</a>
									</c:if>
									...
									<c:if test="${pageNumber<totalPages}">
										<a href="showThesis?pageNumber=${pageNumber+1}">下一页</a>
									</c:if>
									<a href="showThesis?pageNumber=${totalPages}">末页</a>
								</form></td>
						</tr>
					</tfoot>
				</div>
